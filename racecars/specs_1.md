...
* **Track (sim):**
  ...
  * metody:
    ...
    * `finish_crossing_t(p0, p1) -> float | None` — vrátí parametrický okamžik průjezdu cílem (viz sekce 6)
* **Car (sim):**
  ...
  * `penalty: int`
  * `eliminated: bool` (trvale vyřazeno při `--penalty-length fatal`)
  * `laps: int` (počet přejezdů cílem)
  * `crashes: int` (počet nárazů)
  * `distance: float` (celková ujetá vzdálenost v jednotkách mřížky; součet délek segmentů)
  * `controller_name: str` (jméno skriptu/controlleru, nastavuje se při inicializaci)
  * `finish_round: int | None` (kolo, ve kterém auto poprvé projelo cílem)
  * `finish_t: float | None` (parametrický okamžik průjezdu t ∈ [0,1]; viz sekce 6)
...
### 5.1 Fatální srážka (`--penalty-length fatal`)

* místo penalizace čekáním je auto **trvale vyřazeno** ze závodu
* `vel := (0,0)`, žádné penalizační kola se nenastavují
* vyřazené auto **zůstává viditelné** jako vrak (šedý kruh + křížek v původní barvě auta)
* vrak je **překážka**: jiné auto, které do něj narazí, je rovněž vyřazeno (platí pouze pokud je `car_collision_penalty_enabled = on`)
* pokud jsou vyřazena **všechna auta**, závod končí okamžitě bez vítěze

## 6) Cíl a konec hry

* `segment_crosses_finish(pos, pos')` → auto „dojelo”
* závod **neskončí prvním autem u cíle** — pokračuje, dokud **každé pojízdné auto** (tj. `eliminated == False`) neprojede cílem
* pokud jsou při `--penalty-length fatal` **vyřazena všechna auta** před dojezdem, závod končí bez vítěze
* pokud segment protne cíl, **cílem se stane bod na cílové čáře**

### 6.1 Pořadí výsledků

* primárně: `finish_round` (nižší = dojelo dříve)
* sekundárně: `finish_t` — parametrický okamžik průjezdu cílem v rámci tahu

  * kolo se modeluje jako **simultánní**: každé auto svůj tah „jede” v čase 0..1
  * `t = (vzdálenost start→cíl) / (délka celého tahu)` — ekvivalentní parametru průsečíku
  * rychlé auto může v daném kole „přijet” dřív než pomalé, i když jelo na tahu až po něm
  * výpočet: `finish_crossing_t(old_pos, new_pos)` na Track; viz implementaci v `game_state.py`
* terciárně: pořadí tahu v kole (záloha při numerické shodě t)

### 6.2 Výpis výsledků do konzole

Po skončení závodu se do konzole vypíše tabulka (sloupce oddělené tabulátorem):

```
Pořadí  Kola  Nárazy  Vzdálenost  Jméno auta  Controller
```

* nejprve auta, která dojela (seřazená dle 6.1)
* pak auta trvale vyřazená (`eliminated == True`), která nedojela
